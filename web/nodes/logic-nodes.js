...
(function(global) {
    ...
    // Connection Node - 连接节点 (用于条件分支)
    function ConnectionNode() {
        this.title = "连接";
        this.addInput("", LiteGraph.EVENT);
        this.addOutput("", LiteGraph.EVENT);
        this.addProperty("source_id", "");
        this.addProperty("target_id", "");
        this.addProperty("output_type", "output");
        
        // 设置正确的尺寸属性
        this.size = [140, 100];
        this.min_size = [120, 80];
        this.max_size = [250, 150];
        this.resizable = true;
    ...
    ConnectionNode.title = "连接";
    ConnectionNode.desc = "节点连接控制";

    ConnectionNode.prototype.onExecute = function() {
        // Execution handled by backend
    };
    
    ConnectionNode.prototype.configure = function(info) {
        // 调用父类的 configure 方法
        if (LiteGraph.LGraphNode.prototype.configure) {
            LiteGraph.LGraphNode.prototype.configure.call(this, info);
        }
    ...
    ConnectionNode.prototype.onDrawForeground = function(ctx) {
        if (this.flags.collapsed) return;
        ...
        const typeName = typeNames[this.properties.output_type] || this.properties.output_type;
        ctx.fillText(`类型: ${typeName}`, 10, y_offset);
    };

    LiteGraph.registerNodeType("autoclick/connection", ConnectionNode);
    ...
    Object.assign(ConnectionNode.prototype, commonLogicNodeMethods);
